...
/*   Updated: 2024/10/25 16:33:29 by insoares         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_exec(char* av, char** env, int* p)// child - 1o comando, o env e o pipe 1
{
	char **cmd;
	char *path;
	...
	if(execve(path, cmd, env) == -1)// execucao do processo -- se houver erro o execve retorne -1
	{
		ft_free(cmd);
		ft_free(&path);
		ft_close(p);
